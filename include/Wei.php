...
class Wei
{
	...
	public function get_user_info($open_id)
	{
		...
		PubFun::save_log('$is_pass->'.$is_pass.',uid->'.$this->uid.',openid->'.$open_id,'get_user_info');
		...
		{s("aa");
			...
			s($rt_info);
			$rt_info=mb_convert_encoding($rt_info,"UTF-8","auto");
		...
		if(!$access_token)
		{
			if($this->is_auth && $this->AppSecret==''){
			    ...
			    if($authorizer_token){
			    	...
			    	$wxuseraccount->update_data($tokendata," where account_type=1 and userid=".$this->uid);
			    }
			}else{
