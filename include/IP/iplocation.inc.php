...
class ipLocation
{
    ...
    {
        $datfile= dirname(__FILE__)."/".$datfile;
    ...
    function getIP ()
    {
        if ($_SERVER['HTTP_X_FORWARDED_FOR']) { //获取客户端用代理服务器访问时的真实ip 地址  [HTTP_X_FORWARDED_FOR] => 124.65.132.70, 114.80.149.227
            $ip = $_SERVER['HTTP_X_FORWARDED_FOR'];
			$ip = explode(',',$ip);
			$ip = trim($ip[0]);
        } elseif ($_SERVER['X-Real-IP']) {
